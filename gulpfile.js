...
gulp.task("rebuild-assets", async function () {
  var assetGroupTasks = getAssetGroups().map(function (assetGroup) {
    var doRebuild = true;
    return createAssetGroupTask(assetGroup, doRebuild);
  });
...
function buildCssPipeline(assetGroup, doConcat, doRebuild) {
  ...
  var minifiedStream = gulp
    ...
    .pipe(gulpif(generateRTL, postcss(postcssPlugins)))
    .pipe(
      minify({
        minify: true,
        minifyHTML: {
...
function buildJsPipeline(assetGroup, doConcat, doRebuild) {
  ...
  var tsCompilerOptions = assetGroup.hasOwnProperty("tsCompilerOptions")
    ? assetGroup.tsCompilerOptions
    : {
        declaration: false,
        noImplicitAny: true,
        noEmitOnError: true,
